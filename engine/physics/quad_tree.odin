...
quadTreeNodeSubdivide :: proc(
    ...
    for _, i in nodes {
        nodes[i] = quadTreeNodeCreate(
            T, maxCapacity, maxDepth, depth + 1, divisions[i]
        )
    }
...
quadTreeInsert :: proc(
    using root: ^QuadTreeNode($T), point: Vec2f, value: ^T
) {
...
quadTreeGet :: proc(
    using root: ^QuadTreeNode($T), target: AABB, result: ^[dynamic]^T
) {
    assert(root != nil)

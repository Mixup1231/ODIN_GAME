...
physicsInit :: proc(
    bounds: AABB, pointCapacity: u32 = 10, depth: u32 = 5, timestep: f32 = 0.02,
    iterations: u32 = 8
) {
    ...
    s_physicsSystem.timestep = timestep
    ...
    s_physicsSystem.staticTree.tree     = quadTreeCreate(StaticPoint, pointCapacity, depth, bounds)
    ...
    s_physicsSystem.dynamicTree.tree     = quadTreeCreate(DynamicPoint, pointCapacity, depth, bounds)
...
getTimestep :: proc() -> f32 {
    return s_physicsSystem.timestep
}
...
updateDynamicTree :: proc() {
    ...
    quadTreeFree(dynamicTree.tree)
    dynamicTree.tree = quadTreeCreate(DynamicPoint, capacity, depth, bounds)

    entities: [dynamic]ecs.Entity = ecs.cachedEntitiesGet(dynamicSig)
    for entity, i in entities {
        point: ^DynamicPoint = ecs.getComponent(DynamicPoint, entity)
        centre: Vec2f = aabbGetCentre(point.body)
        quadTreeInsert(dynamicTree.tree, centre, point)
    }
}
...
updateStaticTree :: proc() {
    ...
    quadTreeFree(staticTree.tree)
    staticTree.tree = quadTreeCreate(StaticPoint, capacity, depth, bounds)
    
    entities: [dynamic]ecs.Entity = ecs.cachedEntitiesGet(staticSig)
    for entity, i in entities {
        point: ^StaticPoint = ecs.getComponent(StaticPoint, entity)
        centre: Vec2f = aabbGetCentre(point.body)
        quadTreeInsert(staticTree.tree, centre, point)
    }
}
...
integrate :: proc() {
    ...
    for entity in entities {
        ...
        quadTreeGet(staticTree.tree, point.body.boundsCheck, &staticOverlap)
        ...
        quadTreeGet(dynamicTree.tree, point.body.boundsCheck, &dynamicOverlap)
...
update :: proc() {
    ...
    for entity in entities {
        point: ^DynamicPoint = ecs.getComponent(DynamicPoint, entity)
        point.body.velocity = point.body.acceleration * timestep
    }
    ...
    for entity in entities {
        ...
        point.body.boundsCheck.position = aabbGetCentre(point.body) - point.body.boundsCheck.size / 2
    }
}
